...
(defun hive-lsp-bridge--ensure-buffer (project-root file-path)
  ...
  (condition-case err
    (if file-path (or (find-buffer-visiting (expand-file-name file-path)) (let* ((buf (find-file-noselect (expand-file-name file-path) t)))
    (with-current-buffer buf
    (when (and (featurep 'lsp-mode) (not (lsp-workspaces)))
    (sit-for 0.1)))
    buf)) (let* ((ws (hive-lsp-bridge--find-workspace project-root)))
    (when ws
    (car (lsp--workspace-buffers ws)))))
  (error nil)))
...
(defun hive-lsp-bridge--with-lsp-context (project-root file-path callback)
  ...
  (condition-case err
    (let* ((buf (hive-lsp-bridge--ensure-buffer project-root file-path)))
    (if buf (with-current-buffer buf
    (if (and (featurep 'lsp-mode) (lsp-workspaces)) (funcall callback buf) (list :error "No active LSP workspace in buffer"))) (list :error (format "Cannot find buffer for %s in %s" (or file-path "project") project-root))))
  (error (list :error (format "LSP context error: %s" (error-message-string err))))))
...
(defun hive-lsp-bridge-status ()
  ...
  (condition-case err
    (if (featurep 'lsp-mode) (let* ((session (lsp-session))
        (workspaces (lsp--session-workspaces session)))
    (list :lsp-available t :workspace-count (length workspaces) :workspaces (mapcar (lambda (ws)
    (list :root (lsp--workspace-root ws) :server-id (when (lsp--workspace-client ws)
    (lsp--client-server-id (lsp--workspace-client ws))))) workspaces))) (list :lsp-available nil :workspace-count 0 :workspaces '()))
  (error (list :error (format "Status check failed: %s" (error-message-string err))))))
...
(defun hive-lsp-bridge-workspaces ()
  ...
  (condition-case err
    ...
    (mapcar (lambda (ws)
    (list :root (lsp--workspace-root ws) :server-id (when (lsp--workspace-client ws)
    (lsp--client-server-id (lsp--workspace-client ws))) :buffer-count (length (lsp--workspace-buffers ws)))) workspaces)) '())
  (error (list :error (format "Workspaces query failed: %s" (error-message-string err))))))
...
(defun hive-lsp-bridge-hover (project-root file-path line column)
  ...
  (hive-lsp-bridge--with-lsp-context project-root file-path (lambda (_buf)
    (condition-case err
    (let* ((params (hive-lsp-bridge--make-position-params file-path line column))
        (result (lsp-request "textDocument/hover" params)))
    (or result (list :error "No hover information")))
  (error (list :error (format "Hover request failed: %s" (error-message-string err))))))))
...
(defun hive-lsp-bridge-definition (project-root file-path line column)
  ...
  (hive-lsp-bridge--with-lsp-context project-root file-path (lambda (_buf)
    ...
    (or result (list :error "No definition found")))
  (error (list :error (format "Definition request failed: %s" (error-message-string err))))))))
...
(defun hive-lsp-bridge-references (project-root file-path line column)
  ...
  (hive-lsp-bridge--with-lsp-context project-root file-path (lambda (_buf)
    ...
    (or result (list :error "No references found")))
  (error (list :error (format "References request failed: %s" (error-message-string err))))))))
...
(defun hive-lsp-bridge-document-symbols (project-root file-path)
  ...
  (hive-lsp-bridge--with-lsp-context project-root file-path (lambda (_buf)
    (condition-case err
    (let* ((params (list :textDocument (list :uri (lsp--path-to-uri (expand-file-name file-path)))))
        (result (lsp-request "textDocument/documentSymbol" params)))
    (or result (list :error "No symbols found")))
  (error (list :error (format "Document symbols request failed: %s" (error-message-string err))))))))
...
(defun hive-lsp-bridge-cursor-info (project-root file-path line column)
  ...
  (hive-lsp-bridge--with-lsp-context project-root file-path (lambda (_buf)
    ...
    (or result (list :error "No cursor info")))
  (error (list :error (format "Cursor info request failed: %s" (error-message-string err))))))))
...
(defun hive-lsp-bridge-server-info (project-root)
  ...
  (hive-lsp-bridge--with-lsp-context project-root nil (lambda (_buf)
    (condition-case err
    (let* ((result (lsp-request "clojure/serverInfo/raw" nil)))
    (or result (list :error "No server info")))
  (error (list :error (format "Server info request failed: %s" (error-message-string err))))))))
